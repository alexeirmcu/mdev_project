# City Interests Endpoint Specification

## Purpose

Expose a read-only API endpoint that returns distinct interest (attribute) values for a given city, enabling clients to discover available interest tags for trip filtering.

## Requirements

### Requirement: GET /api/cities/{cityCode}/interests returns distinct attribute values

The system SHALL expose `GET /api/cities/{cityCode}/interests` that returns a JSON array of distinct `PlaceAttribute.Value` strings for all places belonging to the specified city. The endpoint MUST delegate to `IPlaceRepository.GetDistinctAttributeValuesByCityCodeAsync` and return results as `InterestsResponse` with an `Interests` property of type `string[]`.

#### Scenario: City with known attributes returns distinct values

- GIVEN a city "madrid-es" with places having attribute values ["museum", "museum", "history", "food", "food"]
...
The API layer SHALL define `InterestsResponse` as a record with a single `Interests` property of type `string[]`. AutoMapper does not apply; the response is constructed directly from the repository result.
