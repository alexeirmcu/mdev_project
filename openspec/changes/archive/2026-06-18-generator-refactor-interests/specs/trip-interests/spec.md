...
`TripPreferences` MUST include an `Interests` property of type `IReadOnlyList<string>`. Interests are matched inclusively against `PlaceAttribute.Value` — a place matches if ANY of its attribute values matches ANY of the trip's interests. Interests MUST be case-insensitive. Empty or null interests list means "no filtering" (backward compatible).
...
- WHEN compared against place attributes "museum" and "food"
